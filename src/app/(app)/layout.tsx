  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside className="hidden lg:flex lg:w-64 lg:flex-col lg:border-r lg:bg-muted/30">
        ...
        <nav className="flex-1 space-y-1 p-4">{sideItems.map((item) => <Link key={item.href} href={item.href} className={cn('flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition', pathname === item.href ? 'bg-rose-50 text-rose-600 font-medium' : 'text-muted-foreground hover:bg-accent hover:text-foreground')}><item.icon className="h-4 w-4" />{item.label}</Link>)}</nav>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around border-t bg-white py-2 lg:hidden">
        {navItems.map((item) => <Link key={item.href} href={item.href} className={cn('flex flex-col items-center gap-0.5 px-3 py-1 text-xs transition', pathname === item.href || (item.href === '/matches' && pathname.startsWith('/chat')) ? 'text-rose-600' : 'text-muted-foreground')}><item.icon className="h-5 w-5" />{item.label}</Link>)}
      </nav>
    </div>
  )
}
