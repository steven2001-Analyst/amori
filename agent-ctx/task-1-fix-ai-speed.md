...
---
## Task ID: 1 - fix-ai-speed-advanced-tools
### Work Task
Fix AI API being extremely slow (195s worst case) and Advanced Tools search failing when AI is unavailable.

### Work Summary

**Issue 1: AI API Too Slow (src/app/api/ai-chat/route.ts)**
- Reduced maxRetries from 2 to 1, SDK timeout from 45000ms to 15000ms, retry delay to flat 1000ms
- Worst case: ~195s reduced to ~31s

**Issue 2: Client-side Timeouts (src/components/ai/ai-assistant-view.tsx)**
- Replaced 3-attempt (45s/60s/90s) with 2-attempt (15s/20s), retry delay 1000ms
- Client worst case: ~36s (down from ~195s)

**Issue 3: Advanced Tools AI Failures (src/components/advanced/advanced-tools-view.tsx)**
- 3 AI API calls identified (AIQuestionGenerator, AITaskGenerator, AIPracticeExercises)
- Each: Added 15s AbortController timeout, null reply check, try/catch with toast.warning + local fallback
- Added fallbackQuestions(), fallbackPlan(), fallbackExercise() for graceful degradation
- Core tools (Regex, JSON, SQL, Colors, Data Converter, Base64, Hash) have ZERO API calls

**Build:** Compiled successfully. **Lint:** Zero errors.

**Files Modified:**
- src/app/api/ai-chat/route.ts (timeout/retry reduction)
- src/components/ai/ai-assistant-view.tsx (timeout/retry reduction)
- src/components/advanced/advanced-tools-view.tsx (3 AI calls with graceful fallbacks)
